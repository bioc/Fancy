...
#'   `find_k = TRUE`.
#' @param cpus Integer; number of parallel workers (default 1). Increase
#'   to parallelise bootstrap iterations across cores.
...
#'
#' @return A [FancyResult-class] S4 object with slots:
#'   \describe{
...
fancy <- function(
  ...
  verbose = TRUE
    ...
    scored <- hybrid_score(edge_metrics,
        w1 = w1, w2 = w2,
        score_type = score_type,
        dcor_rescue_percentile = dcor_rescue_percentile
    )
    all_edges_unfiltered <- hybrid_score(unfiltered_metrics,
        w1 = w1, w2 = w2,
        score_type = score_type,
        dcor_rescue_percentile = dcor_rescue_percentile
    )
    ...
    if (verbose) {
        message(
            "Thresholding edges (method = '", threshold_method,
            "', value = ", threshold_value, ")..."
        )
    }
    edges <- threshold_edges(scored,
    ...
    if (verbose) {
        message(
            "Done. Retained ", nrow(edges), " of ", nrow(scored),
            " edges."
        )
    }
